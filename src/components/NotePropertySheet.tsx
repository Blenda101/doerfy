...
import { X, Lock, Unlock, StickyNote } from 'lucide-react';

interface NotePropertySheetProps {
  item: Note | Notebook;
  onClose: () => void;
  onUpdate: (item: Note | Notebook) => void;
  theme?: Theme;
  notebooks?: Notebook[];
}
        ...
        <div>
          ...
          <div className="grid grid-cols-5 gap-2 mt-2">
            {['blue', 'green', 'purple', 'red', 'yellow'].map((color) => (
              <button
                key={color}
                onClick={() => handleColorThemeChange(color as Note['color_theme'])}
                className={cn(
                  "w-8 h-8 rounded-full",
                  `bg-${color}-500`,
                  item.color_theme === color && "ring-2 ring-offset-2 ring-purple-500"
                )}
              />
            ))}
          </div>
